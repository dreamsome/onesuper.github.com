...
你决定要写一个软件仿真器吗？好极了，这篇文档可能对你有帮助，它涵盖了人们在写仿真器时会提出的一些常见技术问题，同时它也给你了一张模拟器内部的“蓝图”，对你写仿真器多少会有点帮助。


### 什么东西可以被仿真？


基本上只要有微处理器的东西都可以被仿真，当然了，只有那些用来运行软件的设备才值得去仿真，它们包括：
...
这个问题的答案模棱两可，但只要你通过合法途径获取信息，那么仿真有专利的硬件就是合法的。但要注意，若你在发布仿真器的同时发布了有版权保护的系统 ROM（BIOS 等）就是非法的。


### 什么是“解释型”仿真器？它和“重新编译型”仿真器有什么区别？

你可以使用三种基本的模式来实现仿真器，为了得到最佳效果，你可以组合使用这三种模式。
...
但它有一个很明显弱点，性能差。解释需要占用很多 CPU 时间，为了让代码跑出像样的速度，你可能要用一台很快的计算机才行。



#### 静态重新编译型

利用这种技术，你可以把仿真程序翻译成计算机的汇编代码，最后你会得到一个可执行文件，你可以在你的计算机上运行，无需任何特殊工具。虽然静态重新编译听起来不错，但它并非总是可行，例如，如果一个会修改自己的程序，你就没有办法静态重新编译，因为你不知道它会变成什么样，除非你运行它。为了防止这种情况的出现，你可以把静态重新编译器和解释器或动态编译器合在一起使用。


#### 动态重新编译行

动态和静态重新编译本质上是一回事，只是它发生在程序执行时。它会在遇到 CALL 或 JUMP 指令时动态地编译代码，而不是一次性重新编译所的有代码。为了提高速度，这种技术可以和静态重新编译合在一起用。关于动态重新编译的更多内容，你可以阅读 [Ardi 的白皮书](http://www.ardi.com/synpaper.php)，Macintosh 的重新编译型仿真器就是它发明的。
...
1. 选择一种编程语言。
2. 找到和要仿真的硬件有关的一切信息。
3. 自己写 CPU 仿真器或找一份现成的代码。
4. 写一些仿真其他硬件的代码草稿，至少要写一部分。
5. 这时写一个内置的小调试器会很有用，它可以停止仿真然后看到程序在做什么，你可能还需要一个目标系统汇编语言的反汇编器，如果没有就自己写一个。
6. 试着在你的仿真器上运行程序。
...
Cycles[] 表中保存了所有操作码所需花费的 CPU 时钟周期数。**当心**某些操作码（例如条件转移或子例程调用）花费的时钟周期数会根据参数的变化而变化，不过这可以在后续的代码中调整。
...
同时记住在绝大多数情况中调用 ReadMemory() 调用的频率要比 WriteMemory() 高出几倍，因此不妨把更多的代码放到 WriteMemory() 中去实现，而让 ReadMemory() 尽可能简短。 
...
大端序（High-endian）的 CPU 在存储器中保存数据时字（word）的低位字节会先出现。例如，如果你把 0x12345678 保存在大端序的 CPU 时，存储器看起来像是这样：<br>
